...
@Schema(description = "Request to create a new task status")
public class TaskStatusCreateRequest {

    @NotBlank(message = "Status name is required")
    @Size(max = 100, message = "Status name must not exceed 100 characters")
    @Schema(description = "Status name in Vietnamese", example = "Chờ xử lý", required = true)
    String statusName;
    ...
    @Schema(description = "Display order", example = "1", required = true)
    Integer orderIndex;
    ...
    @Schema(description = "Department ID (null for common status)", example = "1")
    ...
    public static class Builder {
        ...
        public TaskStatusCreateRequest build() {
            TaskStatusCreateRequest request = new TaskStatusCreateRequest();
