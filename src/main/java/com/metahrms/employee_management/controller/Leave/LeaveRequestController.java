...
public class LeaveRequestController {
    ...
    // ✅ THÊM endpoint này cho frontend gọi /leave-requests/my
    @GetMapping("/my")
    public ApiResponse<List<LeaveRequestResponseDto>> getMyLeaveRequests() {
        Integer userId = SecurityUtils.getCurrentUserId();
        Employee employee = employeeRepository.findByUserId(userId)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy nhân viên"));

        return ApiResponse.success(
                leaveRequestService.getByEmployee(employee.getId()),
                "Lấy danh sách đơn nghỉ thành công"
        );
    }

    // ✅ Giữ nguyên các endpoint cũ bên dưới
    @PostMapping("/draft")
    public ApiResponse<LeaveRequestResponseDto> createDraft(
            @Valid @RequestBody LeaveRequestCreateDto dto) {
        return ApiResponse.success(
                leaveRequestService.createDraft(dto),
                "Tạo nháp đơn nghỉ thành công"
        );
    }
    ...
    public ApiResponse<LeaveRequestResponseDto> updateDraft(
        ...
        return ApiResponse.success(
                leaveRequestService.updateDraft(id, dto),
    ...
    public ApiResponse<LeaveRequestResponseDto> submit(
            @PathVariable("id") Long id,
            @Valid @RequestBody LeaveSubmitDto dto) {
        return ApiResponse.success(
                leaveRequestService.submit(id, dto),
                "Gửi đơn nghỉ thành công"
        );
    }

    @PutMapping("/{id}/cancel")
    public ApiResponse<LeaveRequestResponseDto> cancel(
            @PathVariable("id") Long id,
            @Valid @RequestBody LeaveCancelDto dto) {
        return ApiResponse.success(
                leaveRequestService.cancel(id, dto),
                "Hủy đơn nghỉ thành công"
        );
    }

    // ⚠️ QUAN TRỌNG: /{id} phải đặt SAU /my
    // Vì nếu đặt trước, Spring sẽ bắt /my vào /{id} gây lỗi
    @GetMapping("/{id}")
    public ApiResponse<LeaveRequestResponseDto> getById(
            @PathVariable("id") Long id) {
        return ApiResponse.success(
    ...
    public ApiResponse<List<LeaveRequestResponseDto>> getByEmployee(
